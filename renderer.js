...
export class Renderer {
    ...
    render(currentMap, player, enemies, projectiles) {
        ...
        // 計算鏡頭偏移量
        const pokotaPxX_on_map_center = player.pxX + this.tileSize / 2;
        const pokotaPxY_on_map_center = player.pxY + this.tileSize / 2;

        const canvasCenterX_Px = this.canvas.width / 2;
        const canvasCenterY_Px = this.canvas.height / 2;

        let offsetX = Math.max(0, Math.min(pokotaPxX_on_map_center - canvasCenterX_Px, currentMap.cols * this.tileSize - this.canvas.width));
        let offsetY = Math.max(0, Math.min(pokotaPxY_on_map_center - canvasCenterY_Px, currentMap.rows * this.tileSize - this.canvas.height));

        // 繪製地圖
        const startCol = Math.floor(offsetX / this.tileSize);
        const endCol = Math.min(currentMap.cols, startCol + GAME_CONSTANTS.CANVAS_WIDTH_TILES + 1);
        const startRow = Math.floor(offsetY / this.tileSize);
        const endRow = Math.min(currentMap.rows, startRow + GAME_CONSTANTS.CANVAS_HEIGHT_TILES + 1);

        for (let y = startRow; y < endRow; y++) {
            for (let x = startCol; x < endCol; x++) {
                const tileData = currentMap.getTile(x, y);
                if (tileData) {
                    switch (tileData.type) {
                        case 0: this._drawGrass(x, y, offsetX, offsetY, tileData.grassShadeIndex, currentMap.colors); break;
                        case 1: this._drawHouse(x, y, offsetX, offsetY); break;
                        case 2: this._drawTree(x, y, offsetX, offsetY); break;
                        case 3: this._drawCarrot(x, y, offsetX, offsetY); break;
                        case 4: this._drawRock(x, y, offsetX, offsetY); break;
                        case 5: this._drawSkull(x, y, offsetX, offsetY); break;
                        case 6: this._drawPortal(x, y, offsetX, offsetY); break;
                    }
                }
            }
        }
        ...
        // 繪製敵人
        enemies.forEach(enemy => enemy.draw(this.ctx, offsetX, offsetY, this.colors));

        // 繪製玩家
        player.draw(this.ctx, offsetX, offsetY, this.colors);

        // 繪製投射物
        projectiles.forEach(p => this._drawProjectile(p, offsetX, offsetY));
    }
}
